...
def init():
    ...
    args_parser.add_argument("--dataset_name",
                             default="2sat_25mixVars_25fixCls_1-hop_100K_muses_500_OR", type=str)
    args_parser.add_argument("--dataset_dir", default="dataset/MUS/25vars_25cls", type=str)
...
def formatting_prompts_func(examples):
    match args.prompt_type:
        case 1:  # Prompt for predicting entailment
            messages = [
                {"role": "system", "content": "The input below provides pairs of a set of premises and a "
        ...
        case 5:
            messages = [
                {"role": "system", "content": "The following input consists of a set of premises (P), "
                ...
                {"role": "user", "content": examples['sentence1']},
                {"role": "system", "content": examples['label']},
            ]
            examples["text"] = tokenizer.apply_chat_template(messages, tokenize=False)
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(args.output_dir, args.model_id, "mus",
                              args.dataset_name + "_prompt_" + str(args.prompt_type))
    ...
    with open(f_preds, 'a') as file_output:
        ...
        for index, row in df_test.iterrows():
            ...
            str_output = str(row['id']) + ";" + str(row['num_clauses']) + ";" + row['label'] \
