...
def init():
    ...
    args_parser.add_argument("--dataset_name", default="2sat_25mixVars_25fixCls_0.3_1-hop_100K_500_OR", type=str)
    args_parser.add_argument("--dataset_dir", default="dataset/Entailments/25vars_25cls", type=str)
    args_parser.add_argument("--model_id", default="unsloth/mistral-7b-instruct-v0.2-bnb-4bit", type=str)
    args_parser.add_argument("--output_dir", default="output", type=str)
    args_parser.add_argument("--prompt_type", default=5, type=int)  # 1:entailment, 2/3/4:inconsistencies, 5:mus
...
def formatting_prompts_func(examples):
    match args.prompt_type:
        case 1:  # Prompt for predicting entailment
            messages = [
                {"role": "user", "content": "The input below provides pairs of a set of premises and a "
                                            "hypothesis. Is the hypothesis entailed by the premises? "
                                            "Answer with yes or no only.\n"
                                            "Premises: " + examples['sentence1'] + " Hypothesis: " +
                                            examples['sentence2'] + "."},
        ...
        case 2:  # Prompt for identifying inconsistency
            messages = [
                {"role": "user", "content": "The input below contains a set of premises. Are there two or "
        ...
        case 3:
            messages = [
                {"role": "user", "content": "The input below contains a set of premises. Is the premises "
        ...
        case 4:
            messages = [
                {"role": "user", "content": "The input below contains a set of premises. If there are any "
        ...
        case 5:
            messages = [
                {"role": "user", "content": "The following input consists of a set of premises (P), "
                                            "some of which are inconsistent. Find the minimal unsatisfiable subset "
                                            "of P. Briefly answer the question by mentioning the MUS premises only. "
                                            "All MUS premises must be exist in P. State the MUS premises "
                                            "consecutively and separated by a period as in the following example: "
                                            "'Premise a. Premise b. ....'. Do not use a new line in the answer.\n"
                                            "Premises: " + examples['sentence1']},
                {"role": "assistant", "content": examples['label']},
            ]
            examples["text"] = tokenizer.apply_chat_template(messages, tokenize=False)
...
def formatting_prompts_test(sentence1, sentence2):
    ...
    match args.prompt_type:
        case 1:  # Prompt for predicting entailment
            messages = [
                {"role": "user", "content": "The input below provides pair of a set of premises and a "
                                            "hypothesis. Is the hypothesis entailed by the premises? "
                                            "Answer with yes or no only.\n"
                                            "Premises: " + sentence1 + " Hypothesis: " + sentence2 + "."},
            ]
        ...
        case 3:
            messages = [
                {"role": "assistant", "content": "The input below contains a set of premises. Is the premises "
                                                 "inconsistent? Answer with yes or no only.\n"
                                                 "Premises: " + sentence1},
            ]
            text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        ...
        case 4:
            messages = [
                {"role": "assistant", "content": "The input below contains a set of premises. If there are any "
                                                 "two or more premises that contradict each other, it means "
                                                 "that the set of premises is inconsistent. "
                                                 "Are the premises inconsistent? Answer with yes or no only.\n"
                                                 "Premises: " + sentence1},
        ...
        case 5:
            ...
            str_text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
...
if __name__ == '__main__':
    ...
    with open(f_preds, 'a') as file_output:
        file_output.write("id;num_clauses;label;pred;k_hop\n")

        for index, row in df_test.iterrows():
            text = formatting_prompts_test(row['sentence1'], row['label'])
            ...
            str_output = str(row['id']) + ";" + str(row['num_clauses']) + ";" + row['label'] + ";" + str(result) + \
                         ";" + str(row['k_hop']) + "\n"
